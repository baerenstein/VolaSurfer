    ...
    with open(file_path, 'r') as file:
        for line in file:
            if 'SPY' in line and ('Bid:' in line and 'Ask:' in line):
                ...
                bid_open, bid_high, bid_low, bid_close = map(float, [parts[bid_index + i] for i in [3, 6, 9, 12]])
                ask_open, ask_high, ask_low, ask_close = map(float, [parts[ask_index + i] for i in [3, 6, 9, 12]])
...
def main(file_path, save_path=None):
    ...
    for timestamp in unique_timestamps:
        print(f"lotting data for {timestamp}")
        sonify_and_plot_volatility_surface(df, timestamp, ax, fig)
        time.sleep(0.05)  # Pause between timestamps
