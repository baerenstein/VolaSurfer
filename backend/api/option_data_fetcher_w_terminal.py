...
def main(save_data=False):
    ...
    try:
        ...
        if contracts_df is not None and final_df is not None:
            ...
            contract_summary = contracts_df.groupby(['expiration_date', 'contract_type']).size()
            ...
            for ticker, count in quotes_by_contract.items():
                ...
                print(f"{ticker} ({contract_info['contract_type'].upper()} ${contract_info['strike_price']:.2f} exp {contract_info['expiration_date']}): {count} quotes")
            ...
            for ticker in final_df['ticker'].unique():
                ...
                print(f"Bid Price Range: ${contract_data['bid_price'].min():.2f} - ${contract_data['bid_price'].max():.2f}")
                print(f"Ask Price Range: ${contract_data['ask_price'].min():.2f} - ${contract_data['ask_price'].max():.2f}")
                print(f"Average Bid-Ask Spread: ${(contract_data['ask_price'] - contract_data['bid_price']).mean():.4f}")
