...
def save_market_data(ticker, results_df, merged_df):
    # Create directory structure
    from pathlib import Path
    data_dir = Path(f"market_data/{ticker}")
    data_dir.mkdir(parents=True, exist_ok=True)
    
    # Save time-indexed data
    current_date = datetime.now().strftime("%Y%m%d")
    
    # Save stock data
    if not results_df.empty:
        results_df.to_parquet(
            data_dir / f"stock_{current_date}.parquet",
            engine='pyarrow'
        )
    ...
    if not merged_df.empty:
        merged_df.to_parquet(
            data_dir / f"options_{current_date}.parquet",
            engine='pyarrow' 
        )

# Call function after your existing DataFrame operations
save_market_data(ticker, results_df, merged_df)