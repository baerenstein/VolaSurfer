...
class NeuralNetworkPricer:
    ...
    def __init__(self, contracts_folder, weights_folder, model_name):
        ...
        Txi = np.concatenate(
            (
                np.arange(0.0025, 0.0175, 0.0025),
        ...
        if model_name == "rheston":
            self.lb = np.concatenate(
                (np.array([0, 0.1, -1]), pow(0.05, 2) * np.ones(27))
            ).reshape(-1, 1)
            self.ub = np.concatenate((np.array([0.5, 1.25, 0]), np.ones(27))).reshape(
                -1, 1
            )
            self.Txi = Txi
            ...
            self.lb = np.concatenate(
                (np.array([0.75, -1, -0.5, -0.5]), pow(0.05, 2) * np.ones(27))
            ).reshape(-1, 1)
            self.ub = np.concatenate(
                (np.array([3.50, 0, 0.5, 0.5]), np.ones(27))
            ).reshape(-1, 1)
            self.Txi = Txi
